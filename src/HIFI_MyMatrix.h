 ...
 public:
  ...
  UpperDiag(int fr,int fc, int lr,int lc, int bs) {
    ...
    for (int i=this->firstRow;i<=this->lastRow;i++) {
      int s=(endCol(i)-startCol(i)); // each of the data[x] is of different size, based on either bandwidth or columns
      this->data[i-this->firstRow]=(T*)malloc(s*sizeof(T)); // * NEED TO EDIT THIS FOR MEMORY ALLOCATION *//
