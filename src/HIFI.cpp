...
float getBias(int i, int j) {
  ...
  //printf("size of bias %d\n",sizeof(bias)/sizeof(bias[0]));
...
int main(int argc, char *argv[]) {
  ...
  bias=(float*) malloc(matrixProperties.fullMatrix_size*sizeof(float));
    ...
    if (rank==0){
        ...
        verticalBoundaries=UpperDiag<char>(options.firstRow,options.firstCol,options.lastRow,options.lastCol, options.bandSize);
  ...
  // Partition and distribute O data, akde guess for initial T, and Ooverbias to all tasks
  MPI_Status status;
  float *bufferOsend;
  float *bufferTsend;
  ...
  if (rank==0){
      ...
      fprintf(stderr,"Communicating O, OoverBias, and T to other ranks\n");
      fflush(stderr);
      int colstart,colend;
  ...
  UpperDiag<char> vertBoundtask=UpperDiag<char>(options.firstRow+rowStart,options.firstCol,options.firstRow+rowEnd,options.lastCol, options.bandSize);

  // allocate buffers for receiving boundary data
  char *bufferhorz;
  bufferhorz=(char*) malloc(sizeof(char)*numElements);
  char *buffervert;
  buffervert=(char*) malloc(sizeof(char)*numElements);
  ...
  if (rank==0){
        ...
        for (int k=0;k<size;k++) {
            ...
            for (int thisrow=allrowstart[k];thisrow<=allrowend[k];thisrow++) { // for the rows on each task
                ...
                for (int thiscol=colstart;thiscol<=colend;thiscol++){
                    ...
                    buffervsend[buffcounter]=verticalBoundaries.get(thisrow+options.firstRow,thiscol);
                    buffcounter++;
                }
            }
            // Send data
            MPI_Isend(&bufferhsend[0],numElemonEachTask[k],MPI_CHAR,k,3,MPI_COMM_WORLD,&req1[k]);
            MPI_Isend(&buffervsend[0],numElemonEachTask[k],MPI_CHAR,k,4,MPI_COMM_WORLD,&req2[k]);
        }
   }
      ...
      if (rank==0) { 
         ...
         MPI_Irecv(&bufferhorz[0],numElements,MPI_CHAR,0,3,MPI_COMM_WORLD,&req3); 
         MPI_Irecv(&buffervert[0],numElements,MPI_CHAR,0,4,MPI_COMM_WORLD,&req4);
         MPI_Wait(&req3,&status);MPI_Wait(&req4,&status);
         MPI_Waitall(size,req1,MPI_STATUSES_IGNORE);MPI_Waitall(size,req2,MPI_STATUSES_IGNORE);
       } else {
  ...
  for (int ii=rowStart;ii<=rowEnd;ii++){ // for each row
      ...
      for (int jj=tempcolstart;jj<=tempcolend;jj++){
          ...
          vertBoundtask.set(ii+options.firstRow,jj,buffervert[communcounter]);
          communcounter++; // iterate through elements
      }
  }
 ...
 // Holds temporary optimized T matrices
  ...
  for (int rep=0;rep<options.mrfMaxIter;rep++) {
      ...
      double sumChanges=0;
        ...
        */
        ...
        for (int i=startIterRow;i<endIterRow;i++){ // Loop through elements
	        ...
	        if (rep==0 || changed) {
	            float medianNeighbor=getMedianNeighbor(Ttask, horzBoundtask, vertBoundtask, i,j);
	            int o=Otask.get(i,j);
	            float newT=optimizeTnew(Otask, Ttask, horzBoundtask, vertBoundtask, i,j, o, medianNeighbor,getBias(i,j));
    ...
    // Communicate Ttask to root to populate full T 
        ...
        // Gather at root. Nonblocking gatherv. Wait at end of iteration.
    ...
    MPI_Waitall(2,reqChanges, MPI_STATUSES_IGNORE); // wait for nChangesTot and sumChangesTot to finish communication
    MPI_Wait(&reqFullT,MPI_STATUS_IGNORE); // Wait for communication of global T to finish
    if (rank==0){     // Organize received global T into UpperDiag format and print results of iteration
        ...
        fprintf(stderr,"***Success reassigning T. Total num assigned %d***\n",recvcountall); 
